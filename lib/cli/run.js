...
function runApp (indexFilePath) {
    var server,
        ...
        appPath = configPath.replace(/index.js$/, ''),
        appNodeModulesPath = appPath + '/node_modules/',
    ...
    function startServer () {
        ...
        var plugins = matchdep
            ...
            .map(function (pluginName) {
                console.log('Found plugin %s'.info, pluginName);
                return {
                    plugin: require(appNodeModulesPath + pluginName),
        ...
        if (plugins.length) {
                    ...
                    server.pack.register(
                        ...
                        function (err) {
                            if (err) {
                                ...
                                console.log(
                                    'Successfully registered plugin %s'.info,
                                    plugin.fullName
                                );
            ...
            async.series(tasks, function (err) {
                ...
                server.start(function () {
                    console.log('Worker server started %s'.info,
                            server.info.uri);
                });
            });
    ...
    if (!cluster.isMaster) {
        ...
        if (process.env.NODE_ENV === 'development') {
            cpuCount = 2;
            console.log(('Reducing cluster worker processes' +
                        ' to %s for development mode').info,
                        cpuCount);
        }
